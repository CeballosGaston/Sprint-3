...
function bestFilmOfYear(array, year) {
  let bestMovie = [];
  const result = array.filter((m) => m.year === year);
  const scoreSorted = result.sort((a, b) => {
    if (a.score > b.score) {
      return -1;
    }
    if (a.score < b.score) {
  ...
  if(scoreSorted.length > 0){
    ...
    const scoreSortedFiltered = scoreSorted.filter((m) => m.score === scoreSorted[0].score);
    bestMovie = scoreSortedFiltered;
  }


  // bestMovie.push(scoreSorted[0]);
  return bestMovie;
}
